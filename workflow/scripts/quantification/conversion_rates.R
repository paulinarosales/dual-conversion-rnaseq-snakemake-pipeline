...
# ---------- Functions ---------- #
computeRates <- function(gene_rates_tab, ref_base) {
...
rownames(geneset) <- geneset$gene_id

gene_rates_tab <- geneset[collapsed_conv$GF,]
...
cat("Merging gene counts assignment with conversion counts...", sep="\n")
ref_code <- c("A", "T", "G", "C") # ignore Ns
for(ref_base in ref_code){
        n_rates_tab <- computeRates(gene_rates_tab, ref_base)
